...
def readRegister(address,i2c):
    try:
        byteval = i2c.readBytes(0x29,address,1)
...
def writeRegister(address, val,i2c):
    try:
        status = i2c.writeByte(0x29, address, val)
...
def powerUp(i2c):
    writeRegister(TSL2561_Control, 0x03,i2c)

def powerDown(i2c):
    writeRegister(TSL2561_Control, 0x00,i2c)

def setTintAndGain(i2c):
    ...
    if timing == 0:
        ...
        timing_ms = 402
    writeRegister(TSL2561_Timing, timing | gain << 4,i2c)

def  readLux(i2c):
    sleep(float(timing_ms + 1) / 1000)

    ch0_low  = readRegister(TSL2561_Channel0L,i2c)
    ch0_high = readRegister(TSL2561_Channel0H,i2c)
    ch1_low  = readRegister(TSL2561_Channel1L,i2c)
    ch1_high = readRegister(TSL2561_Channel1H,i2c)
...
def init(i2c):
    ...
    powerUp(i2c)
    setTintAndGain(i2c)
    writeRegister(TSL2561_Interrupt, 0x00,i2c)
    powerDown(i2c)
